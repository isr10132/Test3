...
public class Painter extends JFrame implements ActionListener {
	...
	private	String menuItem[][]={
		...
		{"上一步(U)|90","下一步(R)|89","剪下(T)|87","複製(C)|68","貼上(P)|85"},
	...
	public Painter(){
		...
		for(i=0;i<menuItem.length;i++){
			for(j=0;j<menuItem[i].length;j++){
				...
				if(i!=2){
					...
					if(menuItem[i][j].split("\\|").length!=1)	//ctrl的快捷鍵
					...
					jMenu[i].add(jMenuItem[i][j]);	//把一欄menuItem放到menu
				}
				//第三個(檢視)為CheckBox
				else{
		...
		for(i=0;i<ButtonName.length;i++){
			...
			jToggleButton[i].setFocusable( false );	//把選定會出現的虛線取消
		...
		//-----------------------設定邊框-----------------------
		//下面的座標軸
		jLabel[0].setBorder(BorderFactory.createBevelBorder(BevelBorder.LOWERED));	//斜面凹進去
		//畫布以外區域
		underDrawPanel.setBorder(BorderFactory.createBevelBorder(BevelBorder.LOWERED));	//斜面凹進去
		underDrawPanel.setBackground(new Color(128,128,128));
		//色塊區域
		jPanel[3].setBorder(BorderFactory.createMatteBorder(1,0,0,0,new Color(172,168,153)));//花色邊框
